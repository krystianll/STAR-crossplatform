...
#if defined(_WIN32)
// --------------------------------------------------------------------------
// Windows stub: STAR's SysV/POSIX shared-memory genome sharing is unavailable
// on native Windows (no shmget/shmat/mmap-SHM). These definitions let STAR
// compile and run normally with the default --genomeLoad NoSharedMemory, which
// never constructs SharedMemory. The shared modes (LoadAndKeep / LoadAndExit /
// LoadAndRemove / Remove) throw a clean SharedMemoryException that
// Genome::HandleSharedMemoryException reports (advising NoSharedMemory).
// --------------------------------------------------------------------------
SharedMemory::SharedMemory(key_t key, bool unloadLast)
    : _shmID(-1), _sharedCounterID(-1), _counterMem(NULL), _mapped(NULL),
      _length(NULL), _sem(NULL), _isAllocator(false), _needsAllocation(true),
      _key(key), _counterKey(key+1), _unloadLast(unloadLast), _err(&cerr)
{}
...
{
    ThrowError(EOPENFAILED, 0);   // shared memory not supported on Windows
}
